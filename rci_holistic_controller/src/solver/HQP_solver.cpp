...
namespace holistic_controller
{
    namespace solver{
        ...
        HQP_solver::HQP_solver(int variables)
        ...
        void HQP_solver::Add_Task(Constraint constraint, int Task_Level){
            int tmp_a, tmp_b;
        ...
        void HQP_solver::Add_Min_Task(Eigen::MatrixXd &Q, Eigen::VectorXd & C, int Task_Level){
            // if(Q.rows() != C.size())
            //     cout<<Task_Level<<" Level Task Did Not applied"<<endl;
        ...
        void HQP_solver::Update_Task(Constraint constraint, int Task_Level){
            int tmp_a, tmp_b;
            tmp_a = constraint.eq_a.rows();
            tmp_b = constraint.eq_b.size();
            if(tmp_a != tmp_b){
        ...
        void HQP_solver::Update_Min_Task(Eigen::MatrixXd &Q, Eigen::VectorXd &C, int Task_Level){
            // if(Q.rows() != C.size())
            //     cout<<Task_Level<<" Level Task Did Not applied"<<endl;
        ...
        void HQP_solver::get_tilda(Eigen::VectorXd & g_tilda){
            m_G_tilda = g_tilda;
        }

        void HQP_solver::set_C(){
            m_C.setZero(m_variables+m_slacks);
        }

        void HQP_solver::Set_Current_State(Eigen::VectorXd q_current, Eigen::VectorXd v_current){
            m_q_current = q_current;
            m_v_current = v_current;
        }

        void HQP_solver::set_ineq(){
            m_Aineq.setZero(0,m_variables+m_slacks);
            m_Bineq.setZero(0);
        }

        Eigen::VectorXd HQP_solver::solve(){
            m_Task_A_equality.setZero(0,m_variables+m_slacks);
            m_Task_B_equality.setZero(0);
            m_rows = 0;
