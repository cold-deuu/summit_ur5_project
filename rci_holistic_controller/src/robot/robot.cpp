...
namespace holistic_controller{
    namespace robot{
        RobotWrapper::RobotWrapper(const std::string & filename, const std::vector<std::string> &)
        {
            pinocchio::urdf::buildModel(filename, m_model);
            m_model_filename = filename;
            m_na = m_model.nv - 3;

            m_S.setZero(this->na() + 3, this->na());
            m_S_dot.setZero(this->na() + 3 , this->na()); 
            this->setMobileParam(0.165, 0.5, 0.05);
        }
        ...
        {   
            pinocchio::getJointJacobian(m_model, data, index, pinocchio::LOCAL_WORLD_ALIGNED, J);
        }
        ...
        {   
            ...
            pinocchio::getJointJacobian(m_model, data, index, pinocchio::LOCAL, J);
            // J = J_tmp * m_S;
        }
